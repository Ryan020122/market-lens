...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Market Lens 2.0｜市場透視",
    page_icon="◉",
    layout="wide",
...
def plotly_base(fig: go.Figure, height: int = 410, y_title: str = "") -> go.Figure:
    fig.update_layout(
        ...
        font=dict(color="#aebbd0", family="Arial, sans-serif", size=12),
        hovermode="x unified",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        xaxis=dict(gridcolor="rgba(145,166,202,.10)", showline=False),
        yaxis=dict(gridcolor="rgba(145,166,202,.10)", title=y_title, zerolinecolor="rgba(145,166,202,.18)"),
    )
    return fig
...
st.sidebar.markdown(
    ...
    <div class="ml-brand">
      ...
      <div><div class="ml-brand-title">Market Lens 2.0</div><div class="ml-brand-sub">Ryan's buy-side research OS</div></div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if page.startswith("⌂"):
    ...
    with cb_cols[0]:
        stat_card("Federal Reserve", f"{fed.iloc[-1]:.2f}%" if not fed.empty else "Add FRED key", "Effective fed funds", "Policy path matters more than the latest decision", "ml-neu")
    with cb_cols[1]:
        stat_card("Bank of England", boe_value, "Bank Rate", "Watch services inflation, wages and vote split", "ml-neu")
        st.link_button("Open official BoE source", boe_url, use_container_width=True)
    with cb_cols[2]:
        stat_card("European Central Bank", ecb_value, "Deposit facility", "Watch wages, energy, euro and fragmentation", "ml-neu")
        st.link_button("Open official ECB source", ecb_url, use_container_width=True)

    st.info("**Daily discipline:** price action → macro driver → primary source → portfolio exposure → disconfirming evidence.")
...
st.markdown(
    f'<div class="ml-footer">Market Lens 2.0 · Refreshed {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} · Educational use only.</div>',
    unsafe_allow_html=True,
)

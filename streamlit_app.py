...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Market Lens 3.1｜市場透視",
    page_icon="◉",
    layout="wide",
...
st.markdown(
    ...
    <style>
      ...
      .ml-live-strip {display:flex;align-items:center;justify-content:space-between;gap:.8rem;flex-wrap:wrap;margin:.25rem 0 .75rem;padding:.68rem .82rem;border:1px solid var(--ml-border);border-radius:14px;background:rgba(16,25,42,.62);}
      .ml-live-left {display:flex;align-items:center;gap:.5rem;font-size:.78rem;color:var(--ml-muted);}
      .ml-live-dot {width:9px;height:9px;border-radius:999px;background:var(--ml-green);box-shadow:0 0 0 0 rgba(85,214,158,.65);animation:mlPulse 1.8s infinite;}
      .ml-live-dot.closed {background:var(--ml-muted);box-shadow:none;animation:none;}
      .ml-live-time {font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--ml-text);font-weight:700;}
      @keyframes mlPulse {0%{box-shadow:0 0 0 0 rgba(85,214,158,.55)}70%{box-shadow:0 0 0 8px rgba(85,214,158,0)}100%{box-shadow:0 0 0 0 rgba(85,214,158,0)}}
      .ml-flash-up {animation:mlFlashUp .75s ease;}
      .ml-flash-down {animation:mlFlashDown .75s ease;}
      @keyframes mlFlashUp {0%{background:rgba(85,214,158,.24)}100%{background:transparent}}
      @keyframes mlFlashDown {0%{background:rgba(255,123,145,.24)}100%{background:transparent}}
      .ml-chart-note {color:var(--ml-muted);font-size:.76rem;margin-top:-.2rem;margin-bottom:.35rem;}
...
def portfolio_table_html(frame: pd.DataFrame) -> str:
    ...
    for _, row in frame.iterrows():
        ...
        direction = str(row.get("Price direction", "flat"))
        flash_class = "ml-flash-up" if direction == "up" else "ml-flash-down" if direction == "down" else ""
        rows.append(
            f'''<div class="ml-position-row ml-performance {flash_class}">
              <div class="ml-position-ticker">{esc(row.get("Ticker", ""))}</div>
              <div class="ml-position-num">{fmt_number(safe_float(row.get("Shares")), 4)}</div>
              <div class="ml-position-num">${fmt_number(safe_float(row.get("Average cost")), 2)}</div>
...
def market_session_label(now_ny: datetime | None = None) -> str:
...
def live_market_snapshot(tickers: tuple[str, ...]) -> pd.DataFrame:
    """Best-effort research quotes using Yahoo minute bars, including pre/after-hours."""
    if not tickers:
        return pd.DataFrame(columns=["Ticker", "Last", "1D %", "As of", "Session"])
    now_ny = datetime.now(ZoneInfo("America/New_York"))
    session = market_session_label(now_ny)
    rows: list[dict[str, Any]] = []
    try:
        raw = yf.download(
            tickers=list(tickers), period="2d", interval="1m", prepost=True,
    ...
    daily = market_history(tickers, period="10d")
    today_ny = now_ny.date()
    for ticker in tickers:
        intraday = close[ticker].dropna() if not close.empty and ticker in close.columns else pd.Series(dtype=float)
        last = float(intraday.iloc[-1]) if len(intraday) else np.nan
        as_of = pd.to_datetime(intraday.index[-1]).to_pydatetime() if len(intraday) else now_ny
        if getattr(as_of, "tzinfo", None) is None:
            as_of = as_of.replace(tzinfo=timezone.utc).astimezone(ZoneInfo("America/New_York"))
        else:
            as_of = as_of.astimezone(ZoneInfo("America/New_York"))

        prior_close = np.nan
        if not daily.empty and ticker in daily.columns:
            ds = daily[ticker].dropna()
            if len(ds):
                dates = pd.to_datetime(ds.index).date
                prior = ds[[d < today_ny for d in dates]]
                if len(prior):
                    prior_close = float(prior.iloc[-1])
                elif len(ds) >= 2:
                    prior_close = float(ds.iloc[-2])
                else:
                    prior_close = float(ds.iloc[-1])
                if not np.isfinite(last):
                    last = float(ds.iloc[-1])
        d1 = (last / prior_close - 1) * 100 if np.isfinite(last) and np.isfinite(prior_close) and prior_close else np.nan
        rows.append({"Ticker": ticker, "Last": last, "1D %": d1, "As of": as_of, "Session": session})
    return pd.DataFrame(rows)
...
def live_economic_calendar(start_iso: str, end_iso: str) -> tuple[pd.DataFrame, str]:
    ...
    try:
        response = requests.get(
            ...
            timeout=18,
            headers={"User-Agent": "Mozilla/5.0 MarketLens/2.6", "Origin": "https://www.tradingview.com"},
        )
        response.raise_for_status()
...
def bls_official_events(start_iso: str, end_iso: str) -> pd.DataFrame:
    ...
    try:
        response = requests.get(
            ...
            timeout=18,
            headers={"User-Agent": "Mozilla/5.0 MarketLens/2.6 (educational dashboard)"},
        )
        response.raise_for_status()
...
def curated_official_events(start_dt: datetime, end_dt: datetime, fred_key: str = "") -> pd.DataFrame:
    ...
    def add_event(
        event_dt: datetime, country: str, title: str, importance: int, source: str, url: str,
        previous: Any = None, actual: Any = None, forecast: Any = None, unit: str = "",
    ) -> None:
        ...
        if start_dt.astimezone(timezone.utc) <= event_utc <= end_dt.astimezone(timezone.utc):
            rows.append({
                ...
                "Actual": actual, "Forecast": forecast, "Previous": previous, "Unit": unit, "Source": source,
    ...
    # FRED ICSA observations are dated for the Saturday week-end and normally
    # released five days later. This populates Actual/Previous even when a
    # third-party calendar feed is delayed or unavailable.
    ...
    latest_claims: Any = None
    prior_claims: Any = None
    latest_release_date = None
    if fred_key:
        claims_series = transform_fred(fred_series("ICSA", fred_key), "thousands")
        if not claims_series.empty:
            latest_claims = f"{claims_series.iloc[-1]:.0f}K"
            prior_claims = f"{claims_series.iloc[-2]:.0f}K" if len(claims_series) >= 2 else None
            latest_observation_date = pd.Timestamp(claims_series.index[-1]).date()
            latest_release_date = latest_observation_date + timedelta(days=5)
    while cursor <= final_date:
        if cursor.weekday() == 3:
            claims_dt = datetime(cursor.year, cursor.month, cursor.day, 8, 30, tzinfo=NEW_YORK_TZ)
            is_latest_release = latest_release_date is not None and cursor == latest_release_date
            add_event(
                claims_dt, "US", "US Initial Jobless Claims", 2,
                "U.S. Department of Labor / FRED", "https://www.dol.gov/ui/data.pdf",
                previous=prior_claims if is_latest_release else latest_claims,
                actual=latest_claims if is_latest_release else None,
                unit="",
            )
...
def merge_calendar_events(live: pd.DataFrame, official: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, official_row in official.iterrows():
        ...
        if match_index is not None:
            ...
            for value_field in ["Actual", "Forecast", "Previous", "Unit"]:
                if _calendar_display(live_row.get(value_field)) == "—" and _calendar_display(official_row.get(value_field)) != "—":
                    live_row[value_field] = official_row.get(value_field)
...
def next_relevant_events(events: pd.DataFrame, count: int = 3, min_importance: int = 2) -> pd.DataFrame:
    """Return future events only; released items belong in recent_releases()."""
    if events.empty:
        return events
    now = pd.Timestamp.now(tz="UTC")
    filtered = events[(events["Importance"] >= min_importance) & (events["DateTime"] >= now)]
    return filtered.sort_values(["DateTime", "Importance"], ascending=[True, False]).head(count)


def recent_releases(events: pd.DataFrame, count: int = 3, min_importance: int = 2, hours: int = 24) -> pd.DataFrame:
    if events.empty:
        return events
    now = pd.Timestamp.now(tz="UTC")
    actual_mask = events["Actual"].map(lambda value: _calendar_display(value) != "—")
    filtered = events[
        (events["Importance"] >= min_importance)
        & (events["DateTime"] < now)
        & (events["DateTime"] >= now - pd.Timedelta(hours=hours))
        & actual_mask
    ]
    return filtered.sort_values(["DateTime", "Importance"], ascending=[False, False]).head(count)
...
st.sidebar.markdown(
    ...
    <div class="ml-brand">
      ...
      <div><div class="ml-brand-title">Market Lens <span class="ml-version">3.1</span></div><div class="ml-brand-sub">Ryan's buy-side research workspace</div></div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if page.startswith("⌂"):
    ...
    briefing_events, briefing_provider = economic_calendar(
        (briefing_now - timedelta(hours=24)).isoformat(),
    ...
    if briefing_next.empty:
        st.info("No upcoming medium/high-importance events were returned. Open Event Calendar and widen the window for official schedules.")
    else:
        ...
        urgent_class = " urgent" if is_imminent else ""
        alert_label = "IMMINENT" if is_imminent else "UPCOMING"
    ...
    briefing_recent = recent_releases(briefing_events, count=2, min_importance=2, hours=24)
    if not briefing_recent.empty:
        st.markdown('<div class="ml-card-title" style="margin-top:.8rem">LATEST MACRO SURPRISE</div>', unsafe_allow_html=True)
        for _, release_row in briefing_recent.iterrows():
            surprise_text, surprise_kind = event_surprise_text(
                str(release_row["Event"]), release_row.get("Actual"), release_row.get("Forecast")
            )
            release_dt = pd.Timestamp(release_row["DateTime"]).to_pydatetime().astimezone(LONDON_TZ)
            st.markdown(
                f'''<div class="ml-event-alert" style="margin-top:.55rem">
                  <div>
                    <div class="ml-event-alert-title"><span class="ml-stars">{stars(int(release_row["Importance"]))}</span> {esc(release_row["Event"])}</div>
                    <div class="ml-event-alert-meta">{release_dt:%d %b · %H:%M} London · Actual <b>{esc(_calendar_display(release_row.get("Actual"), str(release_row.get("Unit") or "")))}</b> · Forecast {esc(_calendar_display(release_row.get("Forecast"), str(release_row.get("Unit") or "")))} · Previous {esc(_calendar_display(release_row.get("Previous"), str(release_row.get("Unit") or "")))}</div>
                    <div class="ml-event-alert-meta" style="margin-top:.25rem">{esc(event_portfolio_link(str(release_row["Event"])))}</div>
                  </div>
                  <div>{badge(surprise_text, surprise_kind)}</div>
                </div>''',
                unsafe_allow_html=True,
            )
    st.caption(f"Calendar feed: {briefing_provider}. Future events are separated from released data. Official BLS, BEA, Fed, BoE, ECB and FRED claims data are used as fallbacks/overlays.")
...
# -----------------------------------------------------------------------------
    ...
    recent = recent_releases(events, count=5, min_importance={"★": 1, "★★": 2, "★★★": 3}[minimum_stars], hours=24)
    if not recent.empty:
        section_label("Latest released data")
        st.markdown(event_list_html(recent.sort_values(["DateTime", "Importance"], ascending=[False, False])), unsafe_allow_html=True)
    ...
    with st.expander("Alert behaviour and limitations"):
        st.markdown(
            ...
            - Press **Refresh data** after a release. The button now also clears the FRED cache, so weekly claims Actual/Previous can update without waiting for normal cache expiry.
            - Future catalysts and recently released data are displayed separately, so a past release will no longer occupy the **Next catalyst** slot.
            """
        )
...
# -----------------------------------------------------------------------------
    ...
    hero(
        ...
        "先管理風險，再決定下一筆交易",
        "持倉、現金、集中度、壓力測試與加減倉模擬放在同一頁。價格只是輸入；倉位才決定你能否留在市場。",
        ["Live prices", "Cash included", "Risk budget", "Stress test", "Position sizing"],
    )

    position_columns = ["Ticker", "Shares", "Average cost", "Currency", "Thesis confidence (1-5)", "Notes"]
    ryan_default_positions = pd.DataFrame([
        {
            "Ticker": "APLD", "Shares": 626.0, "Average cost": 34.93, "Currency": "USD",
            "Thesis confidence (1-5)": 4,
            "Notes": "AI data-centre exposure; monitor funding, delivery, dilution and customer concentration",
        },
        {
            "Ticker": "MU", "Shares": 12.0, "Average cost": 901.96, "Currency": "USD",
            "Thesis confidence (1-5)": 4,
            "Notes": "HBM and memory cycle; monitor pricing, supply discipline and gross margin",
        },
        {
            "Ticker": "NBIS", "Shares": 30.0, "Average cost": 207.45, "Currency": "USD",
            "Thesis confidence (1-5)": 4,
            "Notes": "AI cloud growth; monitor ARR, capacity ramp, funding, utilisation and dilution",
        },
    ])

    if "cash_gbp" not in st.session_state:
        st.session_state["cash_gbp"] = 2699.68
    if "risk_max_position" not in st.session_state:
        st.session_state["risk_max_position"] = 40.0
    if "risk_min_cash" not in st.session_state:
        st.session_state["risk_min_cash"] = 15.0
    if "risk_per_trade" not in st.session_state:
        st.session_state["risk_per_trade"] = 0.50

    top_actions = st.columns([1.25, 1.25, 1.5, 2.2])
    with top_actions[0]:
        if st.button("Load Ryan current portfolio", type="primary", use_container_width=True):
            st.session_state["positions_v2"] = ryan_default_positions.copy()
            st.session_state["cash_gbp"] = 2699.68
            st.success("Current holdings and cash loaded.")
            st.rerun()
    with top_actions[1]:
        cash_gbp = st.number_input(
            "Cash balance (GBP)", min_value=0.0, step=100.0, format="%.2f",
            key="cash_gbp", help="Cash is included in allocation, concentration and stress tests.",
        )
    with top_actions[2]:
        st.caption("Current saved profile")
        st.markdown(f"**£{float(st.session_state['cash_gbp']):,.2f} cash** · APLD / MU / NBIS")
    with top_actions[3]:
        st.info("正股風險管理頁。CFD、期權及孖展不應混入這個長倉風險預算。")
    ...
    positions["Thesis confidence (1-5)"] = pd.to_numeric(
        positions["Thesis confidence (1-5)"], errors="coerce"
    ...
    positions = positions[(positions["Ticker"] != "") & (positions["Shares"] > 0)].drop_duplicates(
        "Ticker", keep="last"
    ).reset_index(drop=True)
    st.session_state["positions_v2"] = positions

    manage_tab, dashboard_tab, risk_tab, simulator_tab = st.tabs([
        "Manage holdings", "Live dashboard", "Risk centre", "Trade simulator"
    ])

    with manage_tab:
        st.markdown(
            '<div class="ml-form-note">可直接新增、修改或刪除持倉。Quick edit 適合一次調整多個 ticker；詳細表單可修改 Notes 與 Thesis confidence。</div>',
            unsafe_allow_html=True,
        )
        mode_cols = st.columns([1, 1, 2])
        with mode_cols[0]:
            quick_edit_open = st.toggle("Quick edit holdings", value=False, key="quick_edit_holdings")
        with mode_cols[1]:
            show_detailed_form = st.toggle("Detailed editor", value=positions.empty, key="detailed_position_editor")

        if quick_edit_open:
            editable = positions.copy()
            if editable.empty:
                editable = pd.DataFrame([{
                    "Ticker": "", "Shares": 0.0, "Average cost": 0.0, "Currency": "USD",
                    "Thesis confidence (1-5)": 3, "Notes": ""
                }])
            edited_positions = st.data_editor(
                editable, num_rows="dynamic", use_container_width=True, hide_index=True,
                key="quick_positions_editor",
                column_config={
                    "Ticker": st.column_config.TextColumn("Ticker", help="例如 APLD、MU、NBIS"),
                    "Shares": st.column_config.NumberColumn("Shares", min_value=0.0, step=0.1, format="%.4f"),
                    "Average cost": st.column_config.NumberColumn("Average cost", min_value=0.0, step=0.01, format="$%.2f"),
                    "Currency": st.column_config.SelectboxColumn("Currency", options=["USD", "GBP", "EUR", "HKD"]),
                    "Thesis confidence (1-5)": st.column_config.NumberColumn("Confidence", min_value=1, max_value=5, step=1),
                    "Notes": st.column_config.TextColumn("Notes"),
                },
            )
            if st.button("Save all changes", type="primary", use_container_width=False):
                clean = edited_positions.copy()
                clean["Ticker"] = clean["Ticker"].astype(str).str.strip().str.upper()
                clean["Shares"] = pd.to_numeric(clean["Shares"], errors="coerce").fillna(0.0)
                clean["Average cost"] = pd.to_numeric(clean["Average cost"], errors="coerce").fillna(0.0)
                clean["Thesis confidence (1-5)"] = pd.to_numeric(
                    clean["Thesis confidence (1-5)"], errors="coerce"
                ).fillna(3).clip(1, 5).astype(int)
                clean["Currency"] = clean["Currency"].fillna("USD").replace("", "USD")
                clean["Notes"] = clean["Notes"].fillna("")
                clean = clean[(clean["Ticker"] != "") & (clean["Shares"] > 0)].drop_duplicates(
                    "Ticker", keep="last"
                ).reset_index(drop=True)
                st.session_state["positions_v2"] = clean[position_columns]
                st.success("Holdings updated.")
                st.rerun()

        if show_detailed_form:
            ...
            with st.form("direct_position_form", clear_on_submit=False):
                ...
                with row_a[0]:
                    if existing_row is not None:
                        ...
                        ticker_choice = st.selectbox(
                            "Ticker", list(WATCHLIST) + ["Custom ticker"],
                        ...
                        ticker_value = st.text_input(
                            "Custom ticker", placeholder="e.g. NVDA", key=f"custom_ticker_{key_suffix}"
                        ) if ticker_choice == "Custom ticker" else ticker_choice
                with row_a[1]:
                    shares_value = st.number_input(
                        "Shares", min_value=0.0,
                        value=float(existing_row["Shares"]) if existing_row is not None else 0.0,
                        step=0.1, format="%.4f", key=f"shares_{key_suffix}"
                    )
                with row_a[2]:
                    average_value = st.number_input(
                        "Average cost", min_value=0.0,
                        value=float(existing_row["Average cost"]) if existing_row is not None else 0.0,
                        step=1.0, format="%.2f", key=f"average_{key_suffix}"
                    )
                with row_a[3]:
                ...
                with row_b[0]:
                    confidence_value = st.slider(
                        "Thesis confidence", 1, 5,
                        int(existing_row["Thesis confidence (1-5)"]) if existing_row is not None else 3,
                        key=f"confidence_{key_suffix}",
                    )
                with row_b[1]:
                    notes_value = st.text_input(
                        "Notes", value=str(existing_row["Notes"]) if existing_row is not None else "",
                        placeholder="Catalyst, risk, next result to monitor…", key=f"notes_{key_suffix}",
                    )
                submit_cols = st.columns([2, 1])
                with submit_cols[0]:
                    save_position = st.form_submit_button("Save / update position", type="primary", use_container_width=True)
                with submit_cols[1]:
                    delete_position = st.form_submit_button(
                        "Delete position", use_container_width=True, disabled=existing_row is None
                    )
            ...
            if save_position:
                ...
                if not ticker_clean or ticker_clean == "CUSTOM TICKER":
                    ...
                    new_row = pd.DataFrame([{
                        "Ticker": ticker_clean, "Shares": float(shares_value),
                        "Average cost": float(average_value), "Currency": currency_value,
                        "Thesis confidence (1-5)": int(confidence_value), "Notes": notes_value,
                    }])
                    ...
                    st.rerun()
            if delete_position and existing_row is not None:
                st.session_state["positions_v2"] = positions[
                    positions["Ticker"] != str(existing_row["Ticker"])
                ].reset_index(drop=True)
                st.rerun()
        ...
        backup_payload = {
            "cash_gbp": float(st.session_state["cash_gbp"]),
            "risk_settings": {
                "max_position_weight_pct": float(st.session_state["risk_max_position"]),
                "minimum_cash_weight_pct": float(st.session_state["risk_min_cash"]),
                "risk_per_trade_pct": float(st.session_state["risk_per_trade"]),
            },
        ...
        with backup_cols[0]:
            st.download_button(
                "Download positions CSV", positions.to_csv(index=False).encode("utf-8"),
                "market_lens_positions.csv", "text/csv", use_container_width=True,
            )
        with backup_cols[1]:
            st.download_button(
                "Download full JSON backup", json.dumps(backup_payload, indent=2).encode("utf-8"),
                "market_lens_backup.json", "application/json", use_container_width=True,
            )

    active = positions.copy()
    if active.empty:
        with dashboard_tab:
            st.info("Add at least one position in Manage holdings.")
        with risk_tab:
            st.info("Risk analytics will appear after positions are added.")
        with simulator_tab:
            st.info("Trade simulation will appear after positions are added.")
    else:
        # One best-effort quote pull feeds the static risk and simulator tabs.
        quote_tickers = tuple(dict.fromkeys(active["Ticker"].tolist() + ["GBPUSD=X"]))
        risk_quotes = live_market_snapshot(quote_tickers)
        quote_map = risk_quotes.set_index("Ticker") if not risk_quotes.empty else pd.DataFrame()
        gbpusd = safe_float(quote_map.loc["GBPUSD=X", "Last"]) if not quote_map.empty and "GBPUSD=X" in quote_map.index else None
        if gbpusd is None or gbpusd <= 0:
            gbpusd = 1.34

        risk_portfolio = active.copy()
        risk_portfolio["Last"] = risk_portfolio["Ticker"].map(
            lambda ticker: safe_float(quote_map.loc[ticker, "Last"]) if not quote_map.empty and ticker in quote_map.index else None
        )
        fallback_prices = market_snapshot(tuple(active["Ticker"].unique())).set_index("Ticker")
        for idx, row in risk_portfolio.iterrows():
            if safe_float(row["Last"]) is None and row["Ticker"] in fallback_prices.index:
                risk_portfolio.at[idx, "Last"] = safe_float(fallback_prices.loc[row["Ticker"], "Last"])
        risk_portfolio["Market value local"] = risk_portfolio["Shares"] * risk_portfolio["Last"]
        risk_portfolio["Market value GBP"] = np.where(
            risk_portfolio["Currency"].eq("GBP"),
            risk_portfolio["Market value local"],
            np.where(risk_portfolio["Currency"].eq("USD"), risk_portfolio["Market value local"] / gbpusd, np.nan),
        )
        risk_portfolio["Cost basis local"] = risk_portfolio["Shares"] * risk_portfolio["Average cost"]
        risk_portfolio["Cost basis GBP"] = np.where(
            risk_portfolio["Currency"].eq("GBP"),
            risk_portfolio["Cost basis local"],
            np.where(risk_portfolio["Currency"].eq("USD"), risk_portfolio["Cost basis local"] / gbpusd, np.nan),
        )
        risk_portfolio["Unrealised P&L GBP"] = risk_portfolio["Market value GBP"] - risk_portfolio["Cost basis GBP"]
        stock_value_gbp = float(risk_portfolio["Market value GBP"].sum(skipna=True))
        cash_value_gbp = float(st.session_state["cash_gbp"])
        total_assets_gbp = stock_value_gbp + cash_value_gbp
        risk_portfolio["Weight %"] = np.where(
            total_assets_gbp > 0, risk_portfolio["Market value GBP"] / total_assets_gbp * 100, np.nan
        )
        cash_weight = cash_value_gbp / total_assets_gbp * 100 if total_assets_gbp > 0 else 0.0
        largest_idx = risk_portfolio["Weight %"].idxmax() if risk_portfolio["Weight %"].notna().any() else None
        largest_ticker = str(risk_portfolio.loc[largest_idx, "Ticker"]) if largest_idx is not None else "—"
        largest_weight = float(risk_portfolio.loc[largest_idx, "Weight %"]) if largest_idx is not None else 0.0
        stock_weights_decimal = risk_portfolio["Weight %"].fillna(0).div(100).tolist() + [cash_weight / 100]
        hhi = float(sum(weight * weight for weight in stock_weights_decimal))
        effective_positions = 1 / hhi if hhi > 0 else np.nan

        with dashboard_tab:
            section_label("Live portfolio mode")
            control_cols = st.columns([1.2, 1, 2.8])
            with control_cols[0]:
                refresh_choice = st.selectbox(
                    "Auto refresh", ["Off", "5 seconds", "10 seconds", "30 seconds"],
                    index=1, key="portfolio_refresh_choice"
                )
            with control_cols[1]:
                manual_refresh = st.button("Refresh now", use_container_width=True)
            interval_map = {"Off": None, "5 seconds": "5s", "10 seconds": "10s", "30 seconds": "30s"}
            run_every = interval_map[refresh_choice]

            @st.fragment(run_every=run_every)
            def render_live_portfolio() -> None:
                prices = live_market_snapshot(tuple(active["Ticker"].unique()))
                portfolio = active.merge(prices[["Ticker", "Last", "1D %", "As of", "Session"]], on="Ticker", how="left")
                previous_prices = st.session_state.get("portfolio_previous_prices", {})
                directions = []
                for _, row in portfolio.iterrows():
                    ticker = str(row["Ticker"])
                    last = safe_float(row.get("Last"))
                    previous = safe_float(previous_prices.get(ticker))
                    directions.append(
                        "up" if last is not None and previous is not None and last > previous
                        else "down" if last is not None and previous is not None and last < previous
                        else "flat"
                    )
                portfolio["Price direction"] = directions
                st.session_state["portfolio_previous_prices"] = {
                    str(row["Ticker"]): safe_float(row.get("Last")) for _, row in portfolio.iterrows()
                }
                ...
                portfolio["P&L %"] = np.where(
                    portfolio["Cost basis"] > 0,
                    portfolio["Unrealised P&L"] / portfolio["Cost basis"] * 100,
                    np.nan,
                )
                total_value_usd = float(portfolio.loc[portfolio["Currency"].eq("USD"), "Market value"].sum(skipna=True))
                total_cost_usd = float(portfolio.loc[portfolio["Currency"].eq("USD"), "Cost basis"].sum(skipna=True))
                total_pnl_usd = total_value_usd - total_cost_usd
                total_pnl_pct = total_pnl_usd / total_cost_usd * 100 if total_cost_usd else np.nan
                session = str(portfolio["Session"].dropna().iloc[0]) if portfolio["Session"].notna().any() else market_session_label()
                timestamps = [value for value in portfolio["As of"].tolist() if isinstance(value, datetime)]
                as_of = max(timestamps) if timestamps else datetime.now(ZoneInfo("America/New_York"))
                dot_class = "" if session != "Market closed" else " closed"
                st.markdown(
                    f'<div class="ml-live-strip"><div class="ml-live-left"><span class="ml-live-dot{dot_class}"></span><b>{esc(session)}</b><span>Yahoo research quote · may be delayed</span></div><div class="ml-live-time">Updated {as_of.strftime("%H:%M:%S ET")}</div></div>',
                    unsafe_allow_html=True,
                )
                summary_cols = st.columns(4)
                with summary_cols[0]:
                    stat_card("Total assets", f"£{total_assets_gbp:,.2f}", f"GBP/USD {gbpusd:.4f}", "Stocks + cash", "ml-neu")
                with summary_cols[1]:
                    stat_card("USD stock P&L", f"${total_pnl_usd:,.2f}", fmt_pct(total_pnl_pct), "Before fees and tax", delta_class(total_pnl_usd))
                with summary_cols[2]:
                    stat_card("Cash", f"£{cash_value_gbp:,.2f}", f"{cash_weight:.1f}%", "Risk capacity", "ml-neu")
                with summary_cols[3]:
                    stat_card("Largest position", largest_ticker, f"{largest_weight:.1f}%", "Including cash", "ml-neg" if largest_weight > float(st.session_state["risk_max_position"]) else "ml-neu")
                section_label("Live position performance")
                st.markdown(portfolio_table_html(portfolio), unsafe_allow_html=True)

                allocation = risk_portfolio[["Ticker", "Market value GBP"]].copy()
                allocation = pd.concat([
                    allocation,
                    pd.DataFrame([{"Ticker": "Cash", "Market value GBP": cash_value_gbp}]),
                ], ignore_index=True)
                pie = go.Figure(go.Pie(
                    labels=allocation["Ticker"], values=allocation["Market value GBP"], hole=.58,
                    textinfo="label+percent",
                    hovertemplate="<b>%{label}</b><br>Value: £%{value:,.2f}<br>Weight: %{percent}<extra></extra>",
                ))
                pie = plotly_base(pie, height=430)
                colors = [TRACE_COLORS.get(ticker, "#7b8aa1") for ticker in allocation["Ticker"]]
                pie.update_traces(marker=dict(colors=colors, line=dict(color="rgba(255,255,255,.14)", width=1)))
                pie.update_layout(
                    showlegend=True, legend=dict(orientation="v", y=.5, x=1.02),
                    annotations=[dict(text=f"£{total_assets_gbp/1000:.1f}K", x=.5, y=.5, font_size=20, showarrow=False)],
                )
                section_label("Allocation including cash")
                st.plotly_chart(pie, use_container_width=True, config={"displaylogo": False})

            render_live_portfolio()

        with risk_tab:
            section_label("Risk policy")
            policy_cols = st.columns(3)
            with policy_cols[0]:
                max_position = st.slider(
                    "Maximum single-position weight", 15.0, 60.0,
                    float(st.session_state["risk_max_position"]), 1.0, key="risk_max_position"
                )
            with policy_cols[1]:
                min_cash = st.slider(
                    "Minimum cash weight", 5.0, 35.0,
                    float(st.session_state["risk_min_cash"]), 1.0, key="risk_min_cash"
                )
            with policy_cols[2]:
                risk_per_trade = st.slider(
                    "Maximum planned loss per new trade", 0.10, 2.00,
                    float(st.session_state["risk_per_trade"]), 0.10, key="risk_per_trade",
                    help="Percentage of total portfolio, not percentage of the trade amount.",
                )

            risk_cards = st.columns(4)
            with risk_cards[0]:
                stat_card("Largest position", largest_ticker, f"{largest_weight:.1f}%", f"Limit {max_position:.0f}%", "ml-neg" if largest_weight > max_position else "ml-pos")
            with risk_cards[1]:
                stat_card("Cash weight", f"{cash_weight:.1f}%", f"Minimum {min_cash:.0f}%", "Liquidity buffer", "ml-neg" if cash_weight < min_cash else "ml-pos")
            with risk_cards[2]:
                stat_card("HHI concentration", f"{hhi:.3f}", f"Effective positions {effective_positions:.1f}", "Lower is more diversified", "ml-neg" if hhi > .30 else "ml-neu")
            with risk_cards[3]:
                ai_exposure = risk_portfolio.loc[risk_portfolio["Ticker"].isin(["APLD", "MU", "NBIS"]), "Market value GBP"].sum() / total_assets_gbp * 100
                stat_card("AI theme exposure", f"{ai_exposure:.1f}%", "APLD + MU + NBIS", "Correlated risk", "ml-neg" if ai_exposure > 75 else "ml-neu")

            if largest_weight > max_position:
                st.error(f"{largest_ticker} is {largest_weight:.1f}% of total assets, above your {max_position:.0f}% limit.")
            if cash_weight < min_cash:
                required_cash = max(0.0, total_assets_gbp * min_cash / 100 - cash_value_gbp)
                st.warning(f"Cash is below policy. Raise approximately £{required_cash:,.0f} to reach {min_cash:.0f}% at current prices.")
            if ai_exposure > 75:
                st.warning("APLD, MU and NBIS are different companies but share AI CapEx, rates and risk-appetite exposure. Treat them as one correlated risk bucket.")

            section_label("Position limit actions")
            action_rows = []
            for _, row in risk_portfolio.iterrows():
                price_gbp = safe_float(row["Market value GBP"] / row["Shares"]) if row["Shares"] else None
                target_value = total_assets_gbp * max_position / 100
                target_shares = math.floor(target_value / price_gbp) if price_gbp and price_gbp > 0 else int(row["Shares"])
                shares_over = max(0.0, float(row["Shares"]) - target_shares)
                action_rows.append({
                    "Ticker": row["Ticker"],
                    "Current weight": float(row["Weight %"]),
                    "Policy limit": max_position,
                    "Shares": float(row["Shares"]),
                    "Shares to sell to limit": shares_over,
                    "Cash released GBP": shares_over * price_gbp if price_gbp else np.nan,
                })
            ...
            st.dataframe(
                ...
                use_container_width=True,
                column_config={
                    "Current weight": st.column_config.NumberColumn(format="%.1f%%"),
                    "Policy limit": st.column_config.NumberColumn(format="%.1f%%"),
                    "Shares": st.column_config.NumberColumn(format="%.2f"),
                    "Shares to sell to limit": st.column_config.NumberColumn(format="%.0f"),
                    "Cash released GBP": st.column_config.NumberColumn(format="£%.0f"),
                },
            )

            section_label("Stress test")
            stress_presets = {
                "Mild AI de-rating": {"APLD": -15.0, "MU": -10.0, "NBIS": -15.0},
                "Rates shock": {"APLD": -22.0, "MU": -12.0, "NBIS": -18.0},
                "Severe AI unwind": {"APLD": -30.0, "MU": -22.0, "NBIS": -30.0},
                "Company execution shock": {"APLD": -35.0, "MU": -8.0, "NBIS": -20.0},
                "Bull recovery": {"APLD": 25.0, "MU": 18.0, "NBIS": 25.0},
                "Custom": {},
            }
            stress_choice = st.selectbox("Scenario", list(stress_presets), key="stress_scenario")
            shock_frame = risk_portfolio[["Ticker", "Last", "Market value GBP"]].copy()
            shock_frame["Shock %"] = shock_frame["Ticker"].map(stress_presets[stress_choice]).fillna(0.0)
            if stress_choice == "Custom":
                shock_frame = st.data_editor(
                    shock_frame,
                    hide_index=True,
                    use_container_width=True,
                    disabled=["Ticker", "Last", "Market value GBP"],
                    column_config={
                        "Last": st.column_config.NumberColumn(format="$%.2f"),
                        "Market value GBP": st.column_config.NumberColumn(format="£%.2f"),
                        "Shock %": st.column_config.NumberColumn(min_value=-80.0, max_value=200.0, step=1.0, format="%.1f%%"),
                    },
                    key="custom_stress_editor",
                )
            shock_frame["Stressed price"] = shock_frame["Last"] * (1 + shock_frame["Shock %"] / 100)
            shock_frame["P&L impact GBP"] = shock_frame["Market value GBP"] * shock_frame["Shock %"] / 100
            scenario_impact = float(shock_frame["P&L impact GBP"].sum(skipna=True))
            stressed_total = total_assets_gbp + scenario_impact
            scenario_pct = scenario_impact / total_assets_gbp * 100 if total_assets_gbp else 0.0
            stress_cols = st.columns(3)
            with stress_cols[0]:
                stat_card("Scenario impact", f"£{scenario_impact:,.0f}", f"{scenario_pct:+.1f}%", stress_choice, delta_class(scenario_impact))
            with stress_cols[1]:
                stat_card("Stressed assets", f"£{stressed_total:,.0f}", "Cash unchanged", "Before FX move", delta_class(scenario_impact))
            with stress_cols[2]:
                recovery_needed = (total_assets_gbp / stressed_total - 1) * 100 if stressed_total > 0 and scenario_impact < 0 else 0.0
                stat_card("Recovery needed", f"{recovery_needed:.1f}%", "Back to current value", "Asymmetric drawdown", "ml-neu")
            st.dataframe(
                shock_frame[["Ticker", "Shock %", "Stressed price", "P&L impact GBP"]],
                hide_index=True, use_container_width=True,
                column_config={
                    "Shock %": st.column_config.NumberColumn(format="%.1f%%"),
                    "Stressed price": st.column_config.NumberColumn(format="$%.2f"),
                    "P&L impact GBP": st.column_config.NumberColumn(format="£%.0f"),
                },
            )

            section_label("Portfolio drawdown guardrails")
            reference_value = st.number_input(
                "Reference portfolio value (GBP)", min_value=1.0, value=float(total_assets_gbp),
                step=250.0, format="%.2f", key="portfolio_reference_value",
                help="Set this to the value from which you want to measure the next drawdown.",
            )
            drawdown = (total_assets_gbp / reference_value - 1) * 100 if reference_value else 0.0
            st.metric("Drawdown from reference", f"{drawdown:.2f}%")
            guardrail = (
                "Normal: new trades must still pass position and cash limits."
                if drawdown > -5 else
                "Level 1: stop adding risk; no CFD; review the common factor."
                if drawdown > -8 else
                "Level 2: reduce the highest-risk / most concentrated position and restore cash above policy."
                if drawdown > -12 else
                "Level 3: capital preservation. Raise cash toward 30% and reassess the thesis without anchoring to cost."
            )
            st.info(guardrail)

        with simulator_tab:
            section_label("Before-trade simulator")
            sim_cols = st.columns([1, 1, 1, 1])
            with sim_cols[0]:
                sim_ticker = st.selectbox("Ticker", risk_portfolio["Ticker"].tolist(), key="sim_ticker")
            selected = risk_portfolio.loc[risk_portfolio["Ticker"] == sim_ticker].iloc[0]
            current_price = float(selected["Last"])
            current_shares = float(selected["Shares"])
            current_weight = float(selected["Weight %"])
            with sim_cols[1]:
                side = st.selectbox("Action", ["BUY", "SELL"], key="sim_side")
            with sim_cols[2]:
                trade_shares = st.number_input("Shares", min_value=0.0, value=1.0, step=1.0, key="sim_shares")
            with sim_cols[3]:
                execution_price = st.number_input(
                    "Execution price", min_value=0.01, value=current_price, step=0.5, format="%.2f", key="sim_price"
                )

            currency = str(selected["Currency"])
            execution_price_gbp = execution_price if currency == "GBP" else execution_price / gbpusd
            signed_shares = trade_shares if side == "BUY" else -trade_shares
            new_shares = max(0.0, current_shares + signed_shares)
            cash_change = -signed_shares * execution_price_gbp
            new_cash = cash_value_gbp + cash_change
            new_position_value = new_shares * execution_price_gbp
            new_total_assets = total_assets_gbp
            new_weight = new_position_value / new_total_assets * 100 if new_total_assets else 0.0

            old_cost = current_shares * float(selected["Average cost"])
            if side == "BUY" and new_shares > 0:
                new_average_cost = (old_cost + trade_shares * execution_price) / new_shares
            else:
                new_average_cost = float(selected["Average cost"])

            result_cols = st.columns(5)
            with result_cols[0]:
                stat_card("Shares", f"{current_shares:.0f} → {new_shares:.0f}", side, sim_ticker, "ml-neu")
            with result_cols[1]:
                stat_card("Average cost", f"${new_average_cost:.2f}", f"Was ${float(selected['Average cost']):.2f}", "Weighted average", "ml-neu")
            with result_cols[2]:
                stat_card("Position weight", f"{current_weight:.1f}% → {new_weight:.1f}%", f"Limit {float(st.session_state['risk_max_position']):.0f}%", "Concentration", "ml-neg" if new_weight > float(st.session_state["risk_max_position"]) else "ml-pos")
            with result_cols[3]:
                stat_card("Cash", f"£{cash_value_gbp:,.0f} → £{new_cash:,.0f}", f"{new_cash/new_total_assets*100:.1f}%", "After trade", "ml-neg" if new_cash/new_total_assets*100 < float(st.session_state["risk_min_cash"]) else "ml-pos")
            with result_cols[4]:
                trade_value = abs(trade_shares * execution_price_gbp)
                stat_card("Trade value", f"£{trade_value:,.0f}", f"{trade_value/total_assets_gbp*100:.1f}% of assets", "Before fees", "ml-neu")

            section_label("Planned stop and risk budget")
            stop_cols = st.columns(3)
            with stop_cols[0]:
                stop_price = st.number_input(
                    "Stop / thesis-invalidating price", min_value=0.01,
                    value=max(0.01, execution_price * 0.92), step=0.5, format="%.2f", key="sim_stop"
                )
            with stop_cols[1]:
                planned_loss_local = abs(execution_price - stop_price) * trade_shares
                planned_loss_gbp = planned_loss_local if currency == "GBP" else planned_loss_local / gbpusd
                allowed_loss_gbp = total_assets_gbp * float(st.session_state["risk_per_trade"]) / 100
                stat_card("Planned loss", f"£{planned_loss_gbp:,.0f}", f"Budget £{allowed_loss_gbp:,.0f}", "At stop before slippage", "ml-neg" if planned_loss_gbp > allowed_loss_gbp else "ml-pos")
            with stop_cols[2]:
                price_risk_gbp = abs(execution_price - stop_price) if currency == "GBP" else abs(execution_price - stop_price) / gbpusd
                max_shares_by_risk = math.floor(allowed_loss_gbp / price_risk_gbp) if price_risk_gbp > 0 else 0
                stat_card("Max shares by risk", f"{max_shares_by_risk}", f"At {float(st.session_state['risk_per_trade']):.2f}% portfolio risk", "Not a recommendation", "ml-neu")

            violations = []
            if side == "BUY" and new_weight > float(st.session_state["risk_max_position"]):
                violations.append("New position weight exceeds your single-stock limit.")
            if side == "BUY" and new_cash / new_total_assets * 100 < float(st.session_state["risk_min_cash"]):
                violations.append("Cash would fall below your minimum buffer.")
            if planned_loss_gbp > allowed_loss_gbp:
                violations.append("Planned stop loss exceeds your per-trade risk budget.")
            if side == "BUY" and sim_ticker in {"APLD", "MU", "NBIS"} and ai_exposure > 75:
                violations.append("This adds to an AI risk bucket that is already above 75% of assets.")

            if violations:
                for violation in violations:
                    st.error(violation)
                st.markdown("**Trade gate: FAIL** — resize, wait, or fund it by reducing correlated exposure.")
            else:
                st.success("Trade gate: PASS — the trade fits the current numerical guardrails. It can still be a bad investment; this only checks sizing.")

            section_label("FOMO checklist")
            fomo_1 = st.checkbox("I can explain what changed versus expectations, not merely that the price went up.", key="fomo_1")
            fomo_2 = st.checkbox("I know the thesis-invalidating condition before buying.", key="fomo_2")
            fomo_3 = st.checkbox("A 20% overnight gap would not force me to sell another asset or add leverage.", key="fomo_3")
            fomo_4 = st.checkbox("I would still make this trade if the ticker were down 3% today instead of up.", key="fomo_4")
            if all([fomo_1, fomo_2, fomo_3, fomo_4]) and not violations:
                st.success("Process check passed.")
            else:
                st.warning("Do not let '買少了' become the reason for a trade. A position should be sized from risk backward, not regret forward.")
...
st.markdown(
    f'<div class="ml-footer">Market Lens 3.1 · Refreshed {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} · Educational use only.</div>',
    unsafe_allow_html=True,
)

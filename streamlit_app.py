...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Market Lens 2.7｜市場透視",
    page_icon="◉",
    layout="wide",
...
st.sidebar.markdown(
    ...
    <div class="ml-brand">
      ...
      <div><div class="ml-brand-title">Market Lens <span class="ml-version">2.6</span></div><div class="ml-brand-sub">Ryan's buy-side research workspace</div></div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
# -----------------------------------------------------------------------------
    ...
    section_label("Manage holdings")
    st.markdown('<div class="ml-form-note">可直接新增、修改或刪除持倉。Quick edit 適合一次調整多個 ticker；詳細表單可修改 Notes 與 Thesis confidence。</div>', unsafe_allow_html=True)
    ...
    if quick_edit_open:
        ...
        edited_positions = st.data_editor(
            editable, num_rows="dynamic", use_container_width=True, hide_index=True, key="quick_positions_editor",
            column_config={
        ...
        with qcols[0]:
            if st.button("Save all changes", type="primary", use_container_width=True):
                ...
                clean["Thesis confidence (1-5)"] = pd.to_numeric(clean["Thesis confidence (1-5)"], errors="coerce").fillna(3).clip(1,5).astype(int)
                clean["Currency"] = clean["Currency"].fillna("USD").replace("", "USD")
                clean["Notes"] = clean["Notes"].fillna("")
                clean = clean[(clean["Ticker"] != "") & (clean["Shares"] > 0)].drop_duplicates("Ticker", keep="last").reset_index(drop=True)
    ...
    if show_detailed_form:
        ...
        with st.form("direct_position_form", clear_on_submit=False):
            ...
            with row_a[1]:
                shares_value = st.number_input("Shares", min_value=0.0, value=float(existing_row["Shares"]) if existing_row is not None else 0.0, step=0.1, format="%.4f", key=f"shares_{key_suffix}")
            with row_a[2]:
                average_value = st.number_input("Average cost", min_value=0.0, value=float(existing_row["Average cost"]) if existing_row is not None else 0.0, step=1.0, format="%.2f", key=f"average_{key_suffix}")
            with row_a[3]:
                currencies = ["USD", "GBP", "EUR", "HKD"]
                existing_currency = str(existing_row["Currency"]) if existing_row is not None else "USD"
                currency_value = st.selectbox("Currency", currencies, index=currencies.index(existing_currency) if existing_currency in currencies else 0, key=f"currency_{key_suffix}")
            row_b = st.columns([1, 3])
            with row_b[0]:
                confidence_value = st.slider("Thesis confidence", 1, 5, int(existing_row["Thesis confidence (1-5)"]) if existing_row is not None else 3, key=f"confidence_{key_suffix}")
            with row_b[1]:
                notes_value = st.text_input("Notes", value=str(existing_row["Notes"]) if existing_row is not None else "", placeholder="Catalyst, risk, next result to monitor…", key=f"notes_{key_suffix}")
            submit_cols = st.columns([2, 1])
            with submit_cols[0]:
                save_position = st.form_submit_button("Save / update position", type="primary", use_container_width=True)
            with submit_cols[1]:
                delete_position = st.form_submit_button("Delete position", use_container_width=True, disabled=existing_row is None)
        ...
        if save_position:
            ...
            if not ticker_clean or ticker_clean == "CUSTOM TICKER":
                ...
                new_row = pd.DataFrame([{"Ticker": ticker_clean, "Shares": float(shares_value), "Average cost": float(average_value), "Currency": currency_value, "Thesis confidence (1-5)": int(confidence_value), "Notes": notes_value}])
        ...
        if delete_position and existing_row is not None:
            st.session_state["positions_v2"] = positions[positions["Ticker"] != str(existing_row["Ticker"])].reset_index(drop=True)
            st.rerun()
    ...
    if active.empty:
        st.info("直接在上面的編輯器輸入 Ticker、股數和平均成本，儲存後便會計算市值與未實現 P&L。")
    else:
        section_label("Live portfolio mode")
        control_cols = st.columns([1.2, 1, 2.8])
        with control_cols[0]:
            refresh_choice = st.selectbox("Auto refresh", ["Off", "5 seconds", "10 seconds", "30 seconds"], index=1, key="portfolio_refresh_choice")
        with control_cols[1]:
        ...
        def render_live_portfolio() -> None:
            ...
            for _, r in portfolio.iterrows():
                ticker = str(r["Ticker"]); last = safe_float(r.get("Last")); previous = safe_float(previous_prices.get(ticker))
                directions.append("up" if last is not None and previous is not None and last > previous else "down" if last is not None and previous is not None and last < previous else "flat")
            portfolio["Price direction"] = directions
            st.session_state["portfolio_previous_prices"] = {str(r["Ticker"]): safe_float(r.get("Last")) for _, r in portfolio.iterrows()}
            ...
            total_value = float(portfolio["Market value"].sum(skipna=True)); total_cost = float(portfolio["Cost basis"].sum(skipna=True))
            total_pnl = total_value - total_cost; total_pnl_pct = total_pnl / total_cost * 100 if total_cost else np.nan
            session = str(portfolio["Session"].dropna().iloc[0]) if portfolio["Session"].notna().any() else market_session_label()
            timestamps = [x for x in portfolio["As of"].tolist() if isinstance(x, datetime)]
            ...
            st.markdown(f'<div class="ml-live-strip"><div class="ml-live-left"><span class="ml-live-dot{dot_class}"></span><b>{esc(session)}</b><span>Yahoo research quote · may be delayed</span></div><div class="ml-live-time">Updated {as_of.strftime("%H:%M:%S ET")}</div></div>', unsafe_allow_html=True)

            if (portfolio["Currency"] != "USD").any():
                st.warning("目前 P&L 計算未做 FX conversion；非 USD 持倉的平均成本不能與美元現價直接合併。")
            summary_cols = st.columns(3)
            with summary_cols[0]: stat_card("Market value", f"${total_value:,.2f}", "Live estimate", session, "ml-neu")
            with summary_cols[1]: stat_card("Unrealised P&L", f"${total_pnl:,.2f}", fmt_pct(total_pnl_pct), "Before fees and tax", delta_class(total_pnl))
            with summary_cols[2]:
            ...
            if not weights.empty and weights["Market value"].sum() > 0:
                ...
                pie = go.Figure(go.Pie(
                    labels=weights["Ticker"], values=weights["Market value"], hole=.58,
                    textinfo="label+percent", hovertemplate="<b>%{label}</b><br>Market value: $%{value:,.2f}<br>Weight: %{percent}<extra></extra>",
                ))
                pie = plotly_base(pie, height=430)
                pie.update_traces(marker=dict(colors=[TRACE_COLORS.get(t, "#5f8dd3") for t in weights["Ticker"]], line=dict(color="rgba(255,255,255,.14)", width=1)))
                pie.update_layout(showlegend=True, legend=dict(orientation="v", y=.5, x=1.02), annotations=[dict(text=f"${total_value/1000:.1f}K", x=.5, y=.5, font_size=20, showarrow=False)])
                section_label("Portfolio allocation")
                st.markdown('<div class="ml-chart-note">Pie chart uses current market value. Cash is not included unless added as a separate position.</div>', unsafe_allow_html=True)
...
st.markdown(
    f'<div class="ml-footer">Market Lens 2.6 · Refreshed {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} · Educational use only.</div>',
    unsafe_allow_html=True,
)

...
with st.sidebar.expander("Data settings"):
    fred_key = st.text_input("FRED API key", value=secret_key, type="password", help="Optional; enables official FRED series.")
page = st.sidebar.radio("Navigate", ["Morning Brief", "Macro Dashboard", "News Monitor", "Learning Lab", "Watchlist", "Research Notebook"])
st.sidebar.divider()
st.sidebar.caption("Educational tool, not investment advice. Verify material facts with primary sources.")

if page == "Morning Brief":
    st.title("Morning Brief")
    st.caption("A cross-asset briefing designed to train a buy-side information habit.")
    ...
    st.subheader("Central-bank snapshot")
    ...
    with c2:
        st.markdown(f'<div class="card"><b>Bank of England</b><br><br>Bank Rate: <b>{br}</b><br><span style="opacity:.7">Watch services inflation, wages, energy and vote split.</span></div>', unsafe_allow_html=True); st.link_button("Official BoE source", bu)
    with c3:
        st.markdown(f'<div class="card"><b>European Central Bank</b><br><br>Deposit rate: <b>{er}</b><br><span style="opacity:.7">Watch energy, wages, euro and fragmentation.</span></div>', unsafe_allow_html=True); st.link_button("Official ECB source", eu)
    st.subheader("Daily discipline")
    ...
    selected = st.multiselect("Market series", list(MARKETS), default=["^GSPC", "^SOX", "^VIX", "^TNX", "DX-Y.NYB", "GBPUSD=X", "CL=F"], format_func=lambda x: f"{MARKETS[x]} ({x})")
    ...
    st.subheader("Official macro series")
    if not fred_key:
        st.warning("Add a free FRED API key in the sidebar."); st.code('FRED_API_KEY = "your_key_here"', language="toml")
    else:
        sid = st.selectbox("FRED series", list(FRED), format_func=lambda x: f"{FRED[x]} ({x})")
        s = fred_series(sid, fred_key)
        if s.empty: st.error("FRED data unavailable. Check the key.")
        else: st.line_chart(s.rename(FRED[sid]).to_frame(), height=350); st.metric("Latest", f"{s.iloc[-1]:,.2f}")
    st.subheader("Regime checklist")
    ...
    preset = st.selectbox("Theme", list(presets)); query = st.text_input("GDELT query", presets[preset])
    a, b = st.columns(2); span = a.selectbox("Lookback", ["12h", "1d", "3d", "1w", "2w"], index=2); maximum = b.slider("Maximum articles", 10, 100, 30, 10)
    ...
    if news.empty: st.warning("No live articles returned. Broaden the query or retry later.")
        ...
        for _, row in news.iterrows():
            ...
            with st.container(border=True):
                ...
                if row["URL"]: st.link_button("Open original article", row["URL"])
    ...
    tab1, tab2, tab3 = st.tabs(["Concept library", "Shock transmission", "Mini quiz"])
    with tab1:
        topic = st.selectbox("Concept", list(TOPICS)); definition, watch, chain, mistake = TOPICS[topic]
        st.subheader(topic); st.write(definition)
        c1, c2 = st.columns(2)
        with c1: st.markdown("**What to watch**"); st.write(watch); st.markdown("**Typical transmission**"); st.write(chain)
        with c2: st.markdown("**Common mistake**"); st.warning(mistake); st.markdown("**Exercise**"); st.write("Find a primary-source release. Record consensus, actual, first market reaction, and whether it persisted after 24 hours.")
    with tab2:
    ...
    st.subheader("Exposure map"); st.dataframe(exposure[exposure.Ticker.isin(tickers)], hide_index=True, use_container_width=True)
    ...
    st.subheader("Position log"); edited = st.data_editor(st.session_state.positions, num_rows="dynamic", use_container_width=True); st.session_state.positions = edited
    st.download_button("Download position log", edited.to_csv(index=False).encode("utf-8"), "market_lens_positions.csv", "text/csv")

else:
    st.title("Research Notebook")
    st.caption("Write a thesis that can be disproved, not a collection of bullish facts.")
    ...
    st.download_button("Download research notebook", notes.to_csv(index=False).encode("utf-8"), "market_lens_research_notebook.csv", "text/csv")
    st.subheader("Thesis template")
    st.code("""Asset/theme:

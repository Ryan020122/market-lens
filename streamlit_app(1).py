...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Market Lens 2.1｜市場透視",
    page_icon="◉",
    layout="wide",
...
st.markdown(
    ...
    <style>
      ...
      /* Market Lens 2.1 — premium terminal polish */
      html, body, [class*="css"] {
...
def central_bank_card(name: str, value: str, label: str, context: str, url: str) -> None:
    st.markdown(
        f'''<div class="ml-cb-card">
              ...
              <div class="ml-cb-meta"><b style="color:#dce7f5">{esc(label)}</b> · {esc(context)}</div>
...
st.sidebar.markdown(
    ...
    <div class="ml-brand">
      ...
      <div><div class="ml-brand-title">Market Lens <span class="ml-version">2.1</span></div><div class="ml-brand-sub">Ryan's buy-side research workspace</div></div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    f'<div class="ml-footer">Market Lens 2.1 · Refreshed {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} · Educational use only.</div>',
    unsafe_allow_html=True,
)
